...
{
    case "$1" in
        Miniconda3-*.sh|Miniconda3-*.exe|Anaconda3-*.sh|Anaconda3-*.exe|mihomo-*.gz|mihomo-*.zip|country.mmdb|geoip.metadb|gh_*_linux_*.tar.gz|gh_*_macOS_*.zip|gh_*_windows_*.zip|GitHubCLI*.msi|node-v*.tar.gz|node-v*.tar.xz|node-v*.pkg|node-v*.msi) return 0 ;;
        *) return 1 ;;
    esac
}
