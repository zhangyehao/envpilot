...
Mihomo ????????????????????? GitHub ?? GeoIP ????????????????? envpilot ????????? Mihomo ????????

- `downloads/country.mmdb` -> `~/.config/mihomo/country.mmdb`
- `downloads/geoip.metadb` -> `~/.config/mihomo/geoip.metadb`

`install mihomo` ???? `downloads/` ????????????????? `https://github.com/MetaCubeX/meta-rules-dat/releases/download/latest/` ??????????????????`update-mihomo-cache.yml` ????? Mihomo ????????? GeoIP ???????????? PR?
